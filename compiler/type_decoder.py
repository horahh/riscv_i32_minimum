...
class TypeDecoder:
    ...
    def __get_instruction_decoder(self):
        ...
        for (itype, instructions_metadata) in self.__instruction_set["RV32I"][
            "TYPE"
        ].items():
            # print(instructions_metadata)
            for (instruction_subtype, instruction_list) in instructions_metadata[
                "INSTRUCTIONS"
            ].items():
