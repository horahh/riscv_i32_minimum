...
def get_funct3(token):
    ...
    funct3i_token = {"addi": 0, "slti": 2, "sltiu": 3, "xori": 4, "ori": 6, "andi": 7}
    if token in funct3i_token.keys():
        return funct3i_token[token]
    funct3ji_token = {"jalr": 0}
    funct3i_load_token = {"lb": 0, "lh": 1, "lw": 2, "lbu": 4, "lbb": 5}
...
def get_immediate20_utype(token):
    ...
    immediate_rv = (
        (immediate_1 << immediate_1_offset)
        + (immediate_2 << immediate_2_offset)
        + (immediate_3 << immediate_3_offset)
        + (immediate_4 << immediate_4_offset)
    )
    return immediate_rv
